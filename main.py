...
def studentdetails():
   # query=db.session(f"SELECT * FROM `student`")
   query = Student.query.all() 
   return render_template('studentdetails.html',query=query)

@app.route('/triggers')
def triggers():
    #query=db.session(f"SELECT * FROM `trig`")
...
@login_required
def delete(id):
    #db.session(f"DELETE FROM `student` WHERE `student`.`id`={id}")
...
@login_required
def edit(id):
    #dept=db.session("SELECT * FROM `department`")
    ...
    if request.method=="POST":
        ...
        #query=db.session(f"UPDATE `student` SET `rollno`='{rollno}',`sname`='{sname}',`sem`='{sem}',`gender`='{gender}',`branch`='{branch}',`email`='{email}',`number`='{num}',`address`='{address}'")
        student = Student.query.get(id)
